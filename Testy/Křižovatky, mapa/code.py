...
#        display.scroll(str(forward), brightness=3)
...
class Robot:
    ...
    def controlSpeed(self, regulatedDistance):
        ...
        if self.__regulatorDistance.isTimeout(time):
            ...
            self.motionControl.newVelocityIfChanged(newSpeed, None)

    def getTurnCoef(self):
        x = self.__timeTurn - 1
        return x * x * 0.08 + x * 0.45 + 1.0

    def conditionChangeTurn(self, direction, back, distance):
...
def print_localize():
    ...
    global robotLocalizeY
#    display.clear()
    text = str(robotLocalizeX) + ":" + str(robotLocalizeY)
    display.scroll(text, brightness=3)
...
if __name__ == "__main__":
    ...
    commands = [
        ...
        Constants.FORWARD, Constants.FORWARD, Constants.RIGHT, Constants.RIGHT,
        Constants.FORWARD, Constants.FORWARD, Constants.LEFT, Constants.LEFT,
        Constants.FORWARD, Constants.FORWARD, Constants.RIGHT, Constants.RIGHT,
        Constants.FORWARD, Constants.FORWARD, Constants.FORWARD, Constants.RIGHT,
        Constants.FORWARD, Constants.FORWARD, Constants.FORWARD, Constants.RIGHT,
        Constants.FORWARD, Constants.FORWARD, Constants.FORWARD ]
    commandsIndex = -1

    leftCalibrate = CalibrateFactors(0.05, 110, 70, 11.692, 28.643)
    rightCalibrate = CalibrateFactors(0.05, 110, 70, 12.259, 30.332)
    robot = None
    try:       
        robot = Robot(leftCalibrate, rightCalibrate)
        state = Constants.ST_Start
        stateExecComm = Constants.ST_Start
        robot.lastPrintTime = ticks_ms()
        while not button_a.was_pressed():
            robot.update()
#            robot.rideLine(False, None)
            isEnd = stateMachine()
            if isEnd:
                break
            sleep(1)
