...
class LightsControl:
    ...
    def setDirectionFromVelocity(self):
        ...
        if self.__velocity.angular > 10.0:
            self.__indDirection = Direction.LEFT
        elif self.__velocity.angular < -10.0:
...
class Wheel:
    ...
    def getMinimumSpeed(self):
        # dej mi minimální doprednou rychlost kola robota z kalibrace kol kde je uhlova rychlost
...
class MotionControl:
    ...
    def newVelocityIfChange(self, forward, angular):
...
class Robot:
    ...
    def __init__(self, leftCalibrate, rightCalibrate):
        ...
        self.__sonar = Sonar()
        self.__regulatorDistance = RegulatorP(0.5, 1_000)
    ...
    def drive(self, forward, angular):
    ...
    def controlSpeed(self):
        ...
        if self.__regulatorDistance.isTimeout(time):
            ...
            if self.acceptableDistance(regulatedDistance, distance):
                ...
                newSpeed = self.__regulatorDistance.getOutput(time, -regulatedDistance, -distance)
...
#            self.motionControl.newVelocity(newSpeed, 0)
    ...
    def conditionChangeTurn(self, direction):
        ...
        if self.__oldTurn == direction:
            self.__turnCount += 1
        else:
            self.__oldTurn = direction
            self.__turnCount = 0
        ...
        if direction=="L":
            angular = 30 * self.__turnCount
        elif direction=="R":
            angular = -30 * self.__turnCount
        else:
            if self.__turnCount>100:
                forward = 0
        self.motionControl.newVelocityIfChange(forward, angular)
...
if __name__ == "__main__":
    print("Start")
    leftCalibrate  = CalibrateFactors(1.0, 110, 75, 11.692, 28.643)
    rightCalibrate = CalibrateFactors(1.0, 110, 75, 12.259, 30.332)
    robot = None
    try:
        ...
        while not button_a.was_pressed():
            ...
            if ticks_diff(time, lastPrint) > 1_000:
                lastPrint = time
                print(robot.motionControl.__wheelLeft.__pwm, robot.motionControl.velocity.forward, robot.motionControl.__wheelLeft.getSpeed(Unit.MeterPerSecond), robot.getObstacleDistance())
            sleep(1)
        robot.stop()
        print("Stop")
    except BaseException as e:
