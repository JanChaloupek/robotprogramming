...
class Robot:
    ...
    def __init__(self, leftCalibrate, rightCalibrate):
        ...
        self.__regulatorFollow = RegulatorP(0.5, 500)        
    ...
    def follow(self, point):
        time = ticks_ms()
        if self.__regulatorFollow.isTimeout(time):
            goalX = point[0]
            goalY = point[1]
            actualX = self.motionControl.odometry.x
            actualY = self.motionControl.odometry.y
            actualTheta = self.motionControl.odometry.theta
            deltaX = goalX - actualX
            deltaY = goalY - actualY
            goalTheta = atan2(deltaY, deltaX)
            distance = sqrt(deltaX**2 + deltaY**2)
            deltaTheta = goalTheta - actualTheta
            
            print(distance, deltaTheta)
            if deltaTheta>0:
                self.motionControl.odometry.theta += 0.1