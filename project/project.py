...
#    LineTrackLeft = 0x40
    LineTrackLeft = 0x04
    LineTrackAll = LineTrackRight | LineTrackMiddle | LineTrackLeft
#    LineTrackAll = 0x1C
...
#        print('0x{:x}'.format(self.__data))
#        print ('0x{:x}'.format(Senzors.LineTrackAll))
        if time == 0:
        ...
        if self.__data != self.__dataPrev:
            if self.getSenzor(Senzors.LineTrackRight):
                display.pixel(2, 5, 255)
            else:
                display.pixel(2, 5, 1)
                
            if self.getSenzor(Senzors.ObstaleRight):
                display.pixel(5, 6, 255)
            else:
                display.pixel(5, 6, 1)
                
            if self.getSenzor(Senzors.LineTrackMiddle):
                display.pixel(8, 5, 255)
            else:
                display.pixel(8, 5, 1)
                
            if self.getSenzor(Senzors.ObstaleLeft):
                display.pixel(11, 6, 255)
            else:
                display.pixel(11, 6, 1)

            if self.getSenzor(Senzors.LineTrackLeft):
                display.pixel(14, 5, 255)
            else:
...
class Robot:
    ...
    def __getSpeedFromDistance(self, regulatedDistance):
    ...
    def controlSpeed(self):
        ...
        if self.__regulatorDistance.isTimeout(time):
            newSpeed = self.__getSpeedFromDistance(0.2)
...
#        return x * x * 0.08 + x * 0.45 + 1.0
    ...
    def conditionChangeTurn(self, direction, back, maxSpeed=1):
        # predpokladam ze jsem na care tak pojedu rychleji a nebudu zatacet
        forward = min(0.15, maxSpeed)
        forwardTurn = forward / 2
        angularTurnBase = forward / 7
        angular = 0
        if direction == "0":
    ...
    def rideLine(self, back=False, regulatedDistance=False):
        # reguluj zatáčení podle sledovače čáry
        time = ticks_ms()
        if self.__regulatorTurn.isTimeout(time):
            if regulatedDistance:
                # na dalnici nesmime couvat
                maxSpeed = max(0, self.__getSpeedFromDistance(regulatedDistance))
            else:
                maxSpeed = 1
